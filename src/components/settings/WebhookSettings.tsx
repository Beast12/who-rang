  ...
  const handleTestWebhook = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          'Test webhook sent successfully. Check your dashboard for the test notification.',
      });
    } catch (error) {
  ...
  return (
    <Card className="bg-card border">
      <CardHeader>
        ...
        <CardDescription className="text-muted-foreground">
          Configure your Home Assistant webhook integration with custom endpoint
          and authentication
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
          ...
          <Input
            ...
            placeholder={
              config?.has_token
                ...
                : 'Enter webhook token for security'
            }
        ...
        <div className="flex space-x-2">
          <Button
            ...
            disabled={
              isSaving ||
              (!webhookToken.trim() && webhookPath === config?.webhook_path)
            }
          >
        ...
        <div className="mt-4 p-4 bg-muted rounded-lg">
          ...
          <div className="text-sm text-muted-foreground space-y-1">
            <p>
              • <strong>Base URL:</strong> {config?.public_url}
            </p>
            <p>
              • <strong>Current Path:</strong> {config?.webhook_path}
            </p>
            <p>
              • <strong>Token Status:</strong>{' '}
              {config?.has_token ? 'Configured' : 'Not set'}
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 p-4 bg-muted rounded-lg">
          ...
          <div className="text-sm text-muted-foreground space-y-1">
            <p>
              • Include <code>ai_message</code> and <code>location</code> in
              your POST request
            </p>
            <p>
              • Optional fields: <code>ai_title</code>, <code>weather</code>,{' '}
              <code>image</code>
            </p>
            {config?.has_token && (
              <p>
                • Include token in header:{' '}
                <code>X-Webhook-Token: YOUR_TOKEN</code>
              </p>
            )}
            <p>
              • Content-Type: <code>application/json</code> or{' '}
              <code>multipart/form-data</code> (for images)
            </p>
          </div>
        </div>
      </CardContent>
