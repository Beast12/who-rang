...
import { AlertTriangle, Shield, Eye, Settings, AlertCircle, Server, RefreshCw, Zap } from 'lucide-react';
import { useFaceConfig, useUpdateFaceConfig } from '@/hooks/useFaces';
import { useOllamaModels, useTestOllamaConnection, useRefreshOllamaModels } from '@/hooks/useOllama';
import { useOpenAIModels, useTestOpenAIConnection, useRefreshOpenAIModels } from '@/hooks/useOpenAI';
...
export const FaceRecognitionSettings = ({ open, onOpenChange }: FaceRecognitionSettingsProps) => {
  const { data: config, isLoading, error: configError } = useFaceConfig();
  const updateConfigMutation = useUpdateFaceConfig();
  const { data: ollamaData, isLoading: modelsLoading, error: modelsError } = useOllamaModels();
  const testConnectionMutation = useTestOllamaConnection();
  const refreshModelsMutation = useRefreshOllamaModels();
  
  // OpenAI hooks
  const { data: openaiData, isLoading: openaiModelsLoading, error: openaiModelsError } = useOpenAIModels();
  ...
  const validateSettings = () => {
    if (settings.confidence_threshold && (settings.confidence_threshold < 0.4 || settings.confidence_threshold > 0.9)) {
      setValidationError('Confidence threshold must be between 40% and 90%');
      return false;
    }
    
    if (settings.training_images_per_person && settings.training_images_per_person < 1) {
      setValidationError('Training images per person must be at least 1');
      return false;
    }
    
    if (settings.auto_delete_after_days && settings.auto_delete_after_days < 0) {
  ...
  const aiProviders = [
    { value: 'local', label: 'Local (Ollama)', description: 'Process on your own server' },
    { value: 'openai', label: 'OpenAI Vision', description: 'GPT-4 Vision API' },
    { value: 'claude', label: 'Anthropic Claude', description: 'Claude Vision API' },
    { value: 'gemini', label: 'Google Gemini', description: 'Gemini Vision API' },
    { value: 'google-cloud-vision', label: 'Google Cloud Vision', description: 'Google Cloud API' },
  ];
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Configure face recognition to automatically identify recurring visitors
          </DialogDescription>
        </DialogHeader>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setSettings(prev => ({ ...prev, ai_provider: value as any }))}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  {selectedProvider && (
                    <div className="p-3 rounded-lg bg-muted/50">
                      <div className="flex items-center space-x-2 mb-1">
                        <Badge variant={settings.ai_provider === 'local' ? 'default' : 'secondary'}>
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <Select
                            ...
                            onValueChange={(value) => setSettings(prev => ({ ...prev, ollama_model: value }))}
                          >
                            <SelectTrigger>
                              <SelectValue />
                            </SelectTrigger>
                          ...
                          <p className="text-xs text-muted-foreground">
                            ...
                            {ollamaData?.fallback && 'Could not connect to Ollama - showing default models'}
                            {ollamaData?.error && (
                              <span className="text-destructive">{ollamaData.error}</span>
                            )}
                            {!modelsLoading && !ollamaData?.fallback && !ollamaData?.error && 
                        ...
                        <Alert>
                          ...
                          <AlertDescription className="text-xs">
                            Make sure Ollama is installed and the selected model is downloaded. 
                            Run <code className="bg-muted px-1 rounded">ollama pull {settings.ollama_model || 'llava'}</code> to download the model.
                          </AlertDescription>
                        </Alert>
                      </CardContent>
                    </Card>
                    ...
                    <Card className="border-dashed">
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            ...
                            <Button
                              ...
                              variant="ghost"
                              size="sm"
                              onClick={() => refreshOpenAIModelsMutation.mutate()}
                          ...
                          <Select
                            ...
                            onValueChange={(value) => setSettings(prev => ({ ...prev, openai_model: value }))}
                          >
                            <SelectTrigger>
                              <SelectValue />
                            </SelectTrigger>
                              ...
                              {openaiData?.models?.map((model) => (
                                <SelectItem key={model.value} value={model.value}>
                                  <div className="flex items-center justify-between w-full">
                                    ...
                                    {model.created && (
                                      <Badge variant="outline" className="ml-2 text-xs">
                                        {new Date(model.created * 1000).getFullYear()}
                                      </Badge>
                                    )}
                                  </div>
                                </SelectItem>
                              )) || [
                                <SelectItem key="gpt-4o" value="gpt-4o">GPT-4o (Recommended)</SelectItem>,
                                <SelectItem key="gpt-4o-mini" value="gpt-4o-mini">GPT-4o Mini (Cost-effective)</SelectItem>,
                                <SelectItem key="gpt-4-vision-preview" value="gpt-4-vision-preview">GPT-4 Vision Preview</SelectItem>
                              ]}
                            </SelectContent>
                          </Select>
                          <p className="text-xs text-muted-foreground">
                            {openaiModelsLoading && 'Loading available models...'}
                            {openaiData?.fallback && 'Could not connect to OpenAI - showing default models'}
                            {openaiData?.error && (
                              <span className="text-destructive">{openaiData.error}</span>
                            )}
                            {!openaiModelsLoading && !openaiData?.fallback && !openaiData?.error && 
                        ...
                        <Alert>
                          <AlertTriangle className="h-4 w-4" />
                          <AlertDescription className="text-xs">
                            OpenAI API usage will incur costs. Monitor your usage in the OpenAI dashboard. 
                            GPT-4o is recommended for best accuracy, while GPT-4o Mini offers cost savings.
                          </AlertDescription>
                        </Alert>
                      </CardContent>
                    </Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setSettings(prev => ({ ...prev, training_images_per_person: parseInt(value) }))}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="1">1 image (fast, less accurate)</SelectItem>
                        <SelectItem value="3">3 images (recommended)</SelectItem>
                        <SelectItem value="5">5 images (slower, more accurate)</SelectItem>
                        <SelectItem value="10">10 images (best accuracy)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => setSettings(prev => ({ ...prev, background_processing: checked }))}
                    />
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setSettings(prev => ({ ...prev, auto_delete_after_days: parseInt(value) }))}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="0">Never (manual cleanup only)</SelectItem>
                    ...
                    <div className="flex items-start space-x-2">
                      ...
                      <div className="text-sm">
                        ...
                        <p className="text-amber-700 dark:text-amber-300 mt-1">
                          Face recognition data is processed and stored locally. When using cloud providers, 
                          images may be sent to external services for processing.
                        </p>
                      </div>
                    </div>
                  </div>
